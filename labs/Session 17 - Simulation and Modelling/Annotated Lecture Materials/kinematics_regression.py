...
def plot(ax):
    ...
    # source directory name, path has ....csv at the end
    data = np.genfromtxt(file_name, delimiter=",")  # parse file for data

    vec_x = data[:, 0]  # first column of matrix, store in a vector
    vec_y = data[:, 1]

    a, b, c = fit_quadratic(vec_x, vec_y)  # pass into fit_quadratic

    acceleration = a * 2  # constant in front of t^2 is 1/2 times acceleration
    initial_velocity = b

    print(f"Constant acceleration = {acceleration:.4f} m/s^2")
    print(f"     Initial velocity = {initial_velocity:.4f} m/s")

    x = np.linspace(np.min(vec_x), np.max(vec_x), 1000)  # map solution of
    # quadratic to data points
