...
def validate_maze(maze):
    for y in range(10): #iterate through list of numbers from 0 to 9 (y coordinate)
        for x in range(10): #for each number, iterate through another list from 0 to 9 (x coordinate)
            cell = maze[y, x]
...
def plot_cell_walls(ax, maze):
    for y in range(10): #for every row
        bottom = (9 - y) * 10
        top = bottom + 10
        for x in range(10): #for every column
            left = x * 10
            right = left + 10
            cell = maze[y, x] #grab the cell value
            if cell & 1 == 1: #test if cell & 1 is equal to 1 (north wall)
                lc = LineCollection(
                    [[(left, top), (right, top)]], color="black", linewidth=3
                )
                ax.add_collection(lc)
            if cell & 2 == 2: #east wall
                lc = LineCollection(
                    [[(right, bottom), (right, top)]], color="black", linewidth=3
                )
                ax.add_collection(lc)
            if cell & 4 == 4: #south wall
                lc = LineCollection(
                    [[(left, bottom), (right, bottom)]], color="black", linewidth=3
                )
                ax.add_collection(lc)
            if cell & 8 == 8: #west wall
                lc = LineCollection(
...
def main(file_name):
    maze = load_maze(file_name) #loads the maze contained in file_name

    if not validate_maze(maze): #if it passes validation it will display it
        return
    ...
    with open(file_name + ".pickle", "wb") as file_out: #it then saves the maze as a pickle file
        pickle.dump(maze, file_out, pickle.HIGHEST_PROTOCOL)

    plot_maze(ax, file_name, maze) #plot_maze() function will make a plot of the maze, pass in ax, file,
    # and maze
...
if __name__ == "__main__":
    file_name = None
    if len(sys.argv) == 1: #look for maze.csv if there is no command line argument
        file_name = os.path.dirname(sys.argv[0]) + "/maze.csv"
    else:
        file_name = sys.argv[1] #can run from the command line and pass in name of csv
