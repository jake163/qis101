...
from mpl_toolkits.mplot3d.art3d import Poly3DCollection #imported module poly3dcollection
import sys
import os
...
def plot(ax):
    ...
    facets[0] = (vertices[0], vertices[1], vertices[2], vertices[3])  # Bottom #group the vertices into faces
    facets[1] = (vertices[4], vertices[5], vertices[6], vertices[7])  # Top #use your right hand, curl fingers
    ...
    p = Poly3DCollection(facets, linewidth=3, edgecolors=["Blue"], facecolors=["None"]) #create polygon,
    ...
    ax.set_zlim3d(zmin=0, zmax=100) #set zmin to zero, because polygon sits on the floor
